    ...
    transaction_id:
      typeof search["transaction_id"] === "string"
        ...
        : typeof search["transactionId"] === "string"
          ? (search["transactionId"] as string)
          : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Confirmation de paiement — PICHNET 2026" },
      {
        name: "description",
        content: "Vérification du paiement PayUnit et crédit des votes PICHNET 2026.",
      },
    ],
  }),
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-16">
      ...
      <p className="mt-2 text-muted-foreground">
        Vérification de votre paiement auprès de PayUnit.
      </p>

      {!transactionId && (
        <div className="mt-8 rounded-2xl border border-border bg-card p-5">
          <p className="flex items-center gap-2 text-destructive">
            <XCircle className="h-5 w-5" /> Référence de transaction manquante.
          </p>
      ...
      {error && (
        <div className="mt-8 rounded-2xl border border-destructive/40 bg-card p-5">
          <p className="flex items-center gap-2 text-destructive">
        ...
        <div className="mt-8 rounded-2xl border border-border bg-card p-5">
          {result.status === "paid" ? (
            ...
            <p className="flex items-center gap-2 font-medium text-destructive">
              <XCircle className="h-5 w-5" /> Paiement échoué
            </p>
          ) : (
            <p className="flex items-center gap-2 font-medium">
              <Loader2 className="h-5 w-5" /> Paiement en attente
            </p>
          )}
          <p className="mt-2 text-sm text-muted-foreground">{result.message}</p>
          {result.voteQuantity != null && result.status === "paid" && (
            <p className="mt-3 text-sm">
              {result.votesAdded ? "Crédités" : "Déjà crédités"} :{" "}
              <strong>{result.voteQuantity} votes</strong>
            </p>
          )}
